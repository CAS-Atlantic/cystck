...
Cystck_Object
FT2Font::get_path(Py_State *S)
{
    if (!face->glyph) {
        CystckErr_SetString(S, S->Cystck_RuntimeError, "No glyph loaded");
        return 0;
    }
    ft_outline_decomposer decomposer = {};
    FT_Error error = FT_Outline_Decompose( &face->glyph->outline, &ft_outline_funcs, &decomposer);
    if (error ) {
        CystckErr_SetString(S, S->Cystck_RuntimeError,
    ...
    if (!decomposer.index) {  // Don't append CLOSEPOLY to null glyphs.
      ...
      return Cystck_BuildValue(S, "OO", Cystck_FromPyObject(S, vertices.pyobj()), Cystck_FromPyObject(S, codes.pyobj()));
    }
...
void FT2Font::set_text( Py_State *S,
...
void FT2Font::load_char(Py_State *S, long charcode, FT_Int32 flags, FT2Font *&ft_object, bool fallback = false)
{
    ...
    if (fallback && char_to_font.find(charcode) != char_to_font.end()) {
        ...
        ft_object->load_char(S, charcode, flags, throwaway, false);
    } else if (fallback) {
