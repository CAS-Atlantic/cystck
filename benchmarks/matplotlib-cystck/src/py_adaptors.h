...
{
    Py_State *m_ctx;
    Cystck_Object m_paths;
    Cystck_ssize_t m_npaths;

  public:
    typedef PathIterator path_iterator;

    PathGenerator(Py_State *S, Cystck_Object obj) : m_ctx(NULL), m_paths(0), m_npaths(0) 
    {
        if (!set(S, obj)) {
            throw py::exception();
        }
    ...
    {
        if (m_ctx) {
            Py_XDECREF(Cystck2py(m_paths));
            //Cystck_popC(m_ctx, m_paths);
            m_ctx = NULL;
        }
    }

    int set(Py_State *S, Cystck_Object obj)
    {
        if (!CystckList_Check(S, obj) && !CystckTuple_Check(S, obj)) {
            return 0;
        }

        m_paths = Cystck_Dup(S, obj);
        m_ctx = S; 

        m_npaths = Cystck_Length(S, m_paths);

        return 1;
    }

    Cystck_ssize_t num_paths() const
    {
        return m_npaths;
    }

    Cystck_ssize_t size() const
    {
        return m_npaths;
    }
    ...
    path_iterator operator()(size_t i)
    {
        ...
        Cystck_Object item = Cystck_GetItem_i(m_ctx, m_paths, i % m_npaths);
        if (Cystck_IsNULL(item)) {
            throw py::exception();
        }
        Cystck_pushobject(m_ctx, item);
        if (!convert_path_cystck(m_ctx, item, &path)) {
            Cystck_pop(m_ctx, item);
            throw py::exception();
        }
        Cystck_pop(m_ctx, item);
        return path;
